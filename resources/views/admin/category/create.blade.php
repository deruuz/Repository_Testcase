...
@section('content')
    <div class="w-full p-6">
        <h2 class="text-2xl font-bold mb-6 text-start text-white">Create New Category</h2>
        ...
        <form action="{{ route('admin.category.store') }}" method="POST"
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full p-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:text-white resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
